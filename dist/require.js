    ...
    Polymer({
        ...
        _lookupLink : function(origin, terminal) {
          for( var i = 0; i < CWN.ds.data.links.length; i++ ) {
            if( CWN.ds.data.links[i].properties.origin == origin && CWN.ds.data.links[i].properties.terminus == terminal ) {
              return CWN.ds.data.links[i];
            }
          }
          return null;
        },
    ...
    Polymer({
        ...
        renderMarkers : function(ctx) {
            ...
            for( i = 0; i < subregions.length; i++ ) {
              if( subregions[i] == this.hoverRegion ) continue;
              this.drawRegion(ctx, subregions[i]);
            }
            ...
            // now lets draw hovered stuff
            if( this.hoverRegion ) {
              this.drawRegion(ctx, CWN.ds.regionLookupMap[this.hoverRegion], {
                strokeStyle : 'rgba('+CWN.colors.rgb.red.join(',')+',.8)',
                fillStyle : 'rgba('+CWN.colors.rgb.red.join(',')+',.4)',
                noCenteriod : true
              });
            }
        ...
        drawRegion : function(ctx, region, config) {
          if( !config ) config = {};


          if( !region.geo ) {
            console.log('Ignoring: '+region.name+', no geo info');
            return;
          }
          if( !region.geo.geometry ) {
            console.log('Ignoring: '+region.name+', no geo info');
            return;
          }

          for( j = 0; j < region.simplified.length; j++ ) {
            coord = region.simplified[j];
            ...
            for( j = 0; j < coord.length; j++ ) {
              ...
              try {
                point = this.map.latLngToContainerPoint([coord[j][1], coord[j][0]]);
              } catch(e) {
                debugger;
              }
